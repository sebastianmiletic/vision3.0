  ...
  public PollBatch<SatelliteState> poll() {
    ...
    try {
      ...
      if (records.isEmpty()) {
        ...
        return new PollBatch<>("CelesTrak", fallback, fetchedAt, "OFFLINE_FALLBACK");
      }
      return new PollBatch<>("CelesTrak", records, fetchedAt, "OK");
    } catch (Exception exception) {
      List<SatelliteState> fallback = buildOfflineFallback(fetchedAt);
      LOGGER.warn("CelesTrak polling failed: {}. Using offline fallback catalog ({} satellites).", exception.getMessage(), fallback.size());
      return new PollBatch<>("CelesTrak", fallback, fetchedAt, "OFFLINE_FALLBACK");
    }
  }
